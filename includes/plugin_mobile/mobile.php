        ...
        if(preg_match('#<h2>(.*)</h2>#imsU',$mobile_content,$matches)){
            if(!preg_match('#<h2>#i',$matches[1])){
                if(preg_match('#<a.*href="([^>]+)".*>(.*)</a>#imsU',$matches[1],$link_matches)){
                    ...
                    $mobile_content = str_replace('<!-- mobile page link -->','<a href="'.htmlspecialchars($link_href).'" data-icon="plus">'.htmlspecialchars($link_text).'</a> ',$mobile_content);
                    $title_text = trim(strip_tags(str_replace($link_matches[0],'',$matches[1])));
                    $mobile_content = preg_replace('#<h1 id="mobile_page_title">[^<]*</h1>#imsU','<h1>'.$title_text.'</h1>',$mobile_content);
