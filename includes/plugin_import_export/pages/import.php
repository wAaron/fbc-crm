...
$extra_fields = array();
if(class_exists('module_extra',false) && isset($import_options['extra']) && $import_options['extra']){
    $sql = "SELECT `extra_key` FROM `"._DB_PREFIX."extra` WHERE owner_table = '".mysql_real_escape_string($import_options['extra']['owner_table'])."' AND `extra_key` != '' GROUP BY `extra_key` ORDER BY `extra_key`";
    $extra_fields = qa($sql);
}
...
// for windows hosting:
    ...
    <table class="tableclass tableclass_rows">
        ...
        <?php } ?>
        <tr>
            <td colspan="<?php echo count($import_options['fields'])+count($extra_fields);?>">
                <?php _e('etc...');?>
            </td>
        </tr>
        </tbody>
