...
//        $sql .= " LEFT JOIN `"._DB_PREFIX."newsletter_send_member` sm5 ON ns.send_id = sm5.send_id AND sm5.`unsubscribe_time` > 0";
     ...
     * @return array
     */
    public static function process_send($newsletter_id,$send_id,$send_limit=1,$retry_failures=false){
        $newsletter_id = (int)$newsletter_id;
        $send_id = (int)$send_id;
        ...
        if($status_check['send_id']==$send_id){ //$status_check['status']==_NEWSLETTER_STATUS_PENDING && 
            ...
            if(mysql_num_rows($send_member_list)>0){
                while($send_member = mysql_fetch_assoc($send_member_list)){
                    ...
                    if($retry_failures && isset(self::$failed_retries[$send_member['newsletter_member_id']])){
                        continue;
                    }
    ...
    public static function double_optin_confirmation_link($email,$h=false){
        if($h){
            return md5('s3cret7hash for newsletter confirmation '._UCM_FOLDER.' '.$email);
        }
    ...
    public static function unsubscribe_url($newsletter_id=0,$newsletter_member_id=0,$unsubscribe_send_id=0,$h=false){
        ...
        if($h){
            return md5('s3cret7hash for newsletter unsub! '._UCM_FOLDER.' '.$newsletter_id.'-'.$newsletter_member_id.'='.$unsubscribe_send_id);
        }
    ...
    public static function link_to_link($send_id,$link_id,$newsletter_member_id,$h=false){
        if($h){
            return md5('s3cret7hash for newsletter link '.$newsletter_member_id.' '._UCM_FOLDER.' '.$send_id.' '.$link_id);
        }
        return full_link(_EXTERNAL_TUNNEL_REWRITE.'m.newsletter/h.l/s.'.$send_id.'/l.'.$link_id.'/a.'.$newsletter_member_id.'/hash.'.self::link_to_link($send_id,$link_id,$newsletter_member_id,true));
    }
    public static function link_to_image($send_id,$image_id,$newsletter_member_id,$h=false){
        if($h){
            return md5('s3cret7hash for newsletter image '.$newsletter_member_id.' '._UCM_FOLDER.' '.$send_id.' '.$image_id);
        }
        return full_link(_EXTERNAL_TUNNEL_REWRITE.'m.newsletter/h.i/s.'.$send_id.'/i.'.$image_id.'/a.'.$newsletter_member_id.'/hash.'.self::link_to_image($send_id,$image_id,$newsletter_member_id,true));
    }
        ...
        if(mysql_num_rows($pending)>0){
            while($send = mysql_fetch_assoc($pending)){
                ...
                if($send['send_id']){
                    ...
                    if($start_time > time()){
                        if($debug)echo 'not sending this one yet';
                    }else{
                        ...
                        for($x=0;$x<10;$x++){ // todo: find a better way to run the cron job, eg: a timeout in configuration, or a max sends per cron run.
                            $send_result = module_newsletter::process_send($send['newsletter_id'],$send['send_id'],$newsletter_send_burst_count,false);
    ...
    INSERT INTO `<?php echo _DB_PREFIX;?>newsletter_template` (`newsletter_template_id`, `newsletter_template_name`, `content_url`, `body`, `wizard`, `date_created`, `date_updated`, `create_user_id`, `update_user_id`) VALUES
    (1, 'Boutique', '', '', 1, '2012-03-05 15:32:32', '2012-06-23 15:38:38', 1, 1),
