...
function qa($sql,$cache=true){
    if(_ENABLE_CACHE && $cache && class_exists('module_cache',false)){
        $cache_key = md5($sql);
        if($res = module_cache::get_cached_item($cache_key,$sql)){
            return $res;
        }
    }
	$res = query_to_array(query($sql,(isset($cache_key)?'Caching: ':'')));
    if(_ENABLE_CACHE && $cache && class_exists('module_cache',false)){
...
function get_single($table,$key,$val,$force=false){
	...
	if(class_exists('module_cache',false)){
		...
		$cache_key = md5(serialize(array($key,$table,$val)));
		if($cache = module_cache::get_cached_item($cache_key,$table.','.implode('|',$key).','.implode('|',$val))){
            if(!$force){
                return $cache;
            }
		}
...
function update_insert($pkey,$pid,$table,$data=false,$do_replace=false){
	...
	if($do_replace || !is_numeric($pid) || !$pid){
		...
		$sql .= "`"._DB_PREFIX."$table` SET date_created = NOW(), ";
