...
if(floatval(phpversion()) < $required_php_version){
    $setup_errors=true;
    $errors[]=("I'm sorry, a PHP version of $required_php_version or above is REQUIRED to run this - the current PHP version is: ".floatval(phpversion()) . ". The web hosting provider can usually push a button to upgrade, please contact them");
}

$required_php_version = 5.3;
if(floatval(phpversion()) < $required_php_version){
    $setup_errors=true;
    $errors[]=("PHP version $required_php_version or above RECOMMENDED to run this program - the current PHP version is: ".floatval(phpversion()) . ". The web hosting provider can usually push a button to upgrade, please contact them. You can still try to install this by clicking the Ignore Errors button.");
}
                ...
                <div class="content">
                    ...
                    if(isset($_REQUEST['do_install']) && isset($_REQUEST['licence_code']) && strlen($_REQUEST['licence_code'])>10){
                        ...
                        $post = array(
                            ...
                            'current_version' => 0,
                            'installation_location' => 'http'.((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != '' && $_SERVER['HTTPS']!='off')?'s':'').'://'.$_SERVER['HTTP_HOST'].$default_base_dir,
                            'get_file_contents' => 1,
                            'return_as_zip' => 1,
                        ...
                        if(strlen($result)>100 && strpos($result,'PK')===0){
                            ...
                            $unzip_status = unzip('temp/system.zip',true);
                            ?></pre>
                            <?php if($unzip_status){ ?>
                                <p>
                            ...
                            echo "<p>Error With Licence Code. Please try again.</p>";
                        ...
                        <p>Now it's time to automatically install the latest version of Ultimate Client Manager. <br/>
                            To proceed please enter the <strong>licence purchase code</strong> below.<br/>
                            This code is available in the "licence" file after purchasing, please see the downloads page on CodeCanyon.net (<a href="http://ultimateclientmanager.com/webimages/licence_cert_location.jpg" target="_blank">click here for help</a>). <br/>
                            The licence code will look something like this: 30d91230-a8df-4545-1237-467abcd5b920
                        </p>

                        <h3>Please enter the UCM licence purchase code:</h3>
